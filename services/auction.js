...
async function addAuction(req) {
    ...
    let rq = req.query
    let productQuery = `INSERT INTO product values ('${rq.product_id}','${rq.product_name}','${rq.product_details}','${rq.product_category}', '${rq.product_pic}')`
    await db.query(productQuery);
    let insertQuery = `INSERT INTO auction values ('${rq.auction_id}','${rq.product_id}', null,'UPCOMING', '0','${rq.is_private}','0','${rq.auctioneer_id}','${rq.start_date}', '${rq.end_date}','${rq.start_time}', '${rq.end_time}')`
...
async function displayFeed() {
    ...
    let feedQuery = `SELECT * FROM auction INNER JOIN product ON auction.product_id=product.product_id ORDER BY n_likes DESC,n_bidders DESC`
...
async function displayAuction(auction_id) {
    ...
    let displayQuery = `SELECT * FROM auction INNER JOIN product ON auction.product_id=product.product_id WHERE auction_id='${auction_id}'`
...
async function modifyAuction(req) {
    ...
    let product_id = await db.query(`SELECT product_id FROM auction WHERE auction_id='${auction_id}'`)
    let updateProductQuery = `UPDATE product SET product_name='${rq.product_name}',product_details='${rq.product_details}',product_category='${rq.product_category}',product_pic='${rq.product_pic}' WHERE product_id= '${product_id}'`
