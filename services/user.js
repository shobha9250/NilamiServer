...
async function signUp(req,res) {
	...
	try {
		const rows = await db.query(`SELECT * FROM user_data WHERE email='${inputData.email}'`);
		if(rows.length >= 1){
			...
			db.query(insertQuery).then((result) =>{
				res.cookie('token', jwt.sign(payload,config.jwt.secret,{expiresIn: 3600}));
...
async function login(req, res) {
	...
	try {
		const row = await db.query(`SELECT * FROM user_data WHERE email='${email}'`);
		if(!row){
